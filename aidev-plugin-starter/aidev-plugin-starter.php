 ...
 * Description: Starter-Plugin für AIDevelopment mit CI/CD, Tests, i18n und sauberen Best Practices.
 * Version: 0.1.0
...
// Example option + admin page to demonstrate testing and settings.
...
function aidev_ps_render_settings() {
    if ( ! current_user_can( 'manage_options' ) ) {
        return;
    }
    if ( isset( $_POST['aidev_ps_nonce'] ) && wp_verify_nonce( $_POST['aidev_ps_nonce'], 'aidev-plugin-starter_save' ) ) {
...
// Simple function (for unit testing example).
function aidev_plugin_starter_add( $a, $b ) {
    return (int) $a + (int) $b;
}
